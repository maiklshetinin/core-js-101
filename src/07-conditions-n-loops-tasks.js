...
function doRectanglesOverlap(rect1, rect2) {
  return (rect2.left < rect1.left + rect1.width) && (rect1.left < (rect2.left + rect2.width))
    && (rect2.top < rect1.top + rect1.height) && (rect1.top < rect2.top + rect2.height);
}
 ...
 *
 */
function getIntervalString(...args) {
  return [...args].reduce((res, el, index) => {
    let acc = res;
    switch (index) {
      case 0: acc += el; break;
      case 1: acc = acc[0] > el ? `${el}, ${acc}` : `${acc}, ${el}`; break;
      case 2: acc = el ? `[${acc}` : `(${acc}`; break;
      default: acc += el ? ']' : ')';
    }
    return acc;
  }, '');
...
function isCreditCardNumber(ccn) {
  ...
  const arr = str.split('').slice(0, str.length - 1).reverse();
  ...
  const numberContr = 10 - (res % 10);
  if (lastNumber === 0 && res % 10 === 0) return true;

  if (numberContr === lastNumber) return true;
  return false;
}
...
function isBracketsBalanced(str) {
  ...
  arr.forEach((el) => {
    if (stack.length === 0) {
      stack.push(el);
    } else if (open.indexOf(stack[stack.length - 1]) === close.indexOf(el)
    && close[close.indexOf(el)] === el) stack.pop();
    else stack.push(el);
  });
...
function getMatrixProduct(m1, m2) {
  ...
  const resultM = [];
  if (colM1 !== rowsM2) return false;
  for (let i = 0; i < rowM1; i += 1) resultM[i] = [];
...
function evaluateTicTacToePosition(position) {
  switch (true) {
    ...
    case position[2][0] === 'X' && position[2][1] === 'X' && position[2][2] === 'X':
    ...
    case position[0][2] === 'X' && position[1][1] === 'X' && position[2][0] === 'X': return 'X';

    case position[0][0] === '0' && position[0][1] === '0' && position[0][2] === '0':
    case position[1][0] === '0' && position[1][1] === '0' && position[1][2] === '0':
    case position[2][0] === '0' && position[2][1] === '0' && position[2][2] === '0':
    ...
    case position[0][2] === '0' && position[1][1] === '0' && position[2][0] === '0': return '0';
