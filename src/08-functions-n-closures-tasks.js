...
function logger(func, logFunc) {
  return (...args) => {
    const a = func.toString().split('');
    const b = a.slice(a.indexOf(' ') + 1, a.indexOf('(') + 1).join('');
    const textMessage = `${b}${JSON.stringify(args).slice(1, -1)})`;
    logFunc(`${textMessage} starts`);
    const result = func(...args);
    logFunc(`${textMessage} ends`);
    return result;
  };
}
